...
// Labels and annotations set by the ApplicationSet controller on child Applications.
const (
	// LabelKeyApplicationSetOrphaned is set to "true" on Applications that are no longer
	// generated by their parent ApplicationSet but are retained because the sync policy
	// does not allow deletion (e.g. create-only, create-update).
	LabelKeyApplicationSetOrphaned = "applicationset.argocd.argoproj.io/orphaned"

	// AnnotationKeyApplicationSetOrphanedBy records the ApplicationSet that last managed
	// this Application before it became orphaned, in the format "<namespace>/<name>".
	// This helps operators trace back which ApplicationSet to investigate.
	AnnotationKeyApplicationSetOrphanedBy = "applicationset.argocd.argoproj.io/orphaned-by"
)

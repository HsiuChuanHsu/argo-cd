...
func (r *ApplicationSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	if applicationSetInfo.DeletionTimestamp != nil {
		...
		if !deleteAllowed {
			...
			// Mark all retained Applications as orphaned since they will no longer be managed
			// by any ApplicationSet after the parent is gone.
			if err := r.markOrphanedApplications(ctx, logCtx, applicationSetInfo, nil); err != nil {
				logCtx.Errorf("Failed to mark orphaned applications on ApplicationSet deletion: %v", err)
	...
	if utils.DefaultPolicy(applicationSetInfo.Spec.SyncPolicy, r.Policy, r.EnablePolicyOverride).AllowDelete() {
		...
		// Policy does not allow deletion — label apps that are no longer generated as orphaned.
		// Use validApps so apps in error/invalid state are not treated as actively desired.
		if err = r.markOrphanedApplications(ctx, logCtx, applicationSetInfo, validApps); err != nil {
			logCtx.Errorf("Failed to mark orphaned applications: %v", err)
			return ctrl.Result{}, err
		}
	}
...
// markOrphanedApplications labels Applications that are no longer generated by the ApplicationSet
// but are retained because the sync policy does not allow deletion (e.g. create-only, create-update).
// When an Application re-appears in the generator output, the orphaned label and annotation are removed.
// Pass nil for desiredApplications to mark all current Applications as orphaned (e.g. on AppSet deletion).
func (r *ApplicationSetReconciler) markOrphanedApplications(ctx context.Context, logCtx *log.Entry, applicationSet argov1alpha1.ApplicationSet, desiredApplications []argov1alpha1.Application) error {
	desiredSet := make(map[string]bool, len(desiredApplications))
	for _, app := range desiredApplications {
		desiredSet[app.Name] = true
	}

	current, err := r.getCurrentApplications(ctx, applicationSet)
	if err != nil {
		return fmt.Errorf("failed to get current applications: %w", err)
	}

	for i := range current {
		app := &current[i]
		_, isDesired := desiredSet[app.Name]
		isOrphaned := app.Labels[argov1alpha1.LabelKeyApplicationSetOrphaned] == "true"

		switch {
		case !isDesired && !isOrphaned:
			// App is no longer generated — mark it as orphaned.
			...
			app.Labels[argov1alpha1.LabelKeyApplicationSetOrphaned] = "true"
			app.Annotations[argov1alpha1.AnnotationKeyApplicationSetOrphanedBy] = applicationSet.Namespace + "/" + applicationSet.Name
			if patchErr := r.Patch(ctx, app, patch); patchErr != nil {
				logCtx.Errorf("Failed to mark application %s as orphaned: %v", app.Name, patchErr)
				return patchErr
			}
			logCtx.Infof("Marked application %s as orphaned by %s/%s", app.Name, applicationSet.Namespace, applicationSet.Name)
		case isDesired && isOrphaned:
			// App has returned to the generator output — remove the orphaned markers.
			patch := client.MergeFrom(app.DeepCopy())
			delete(app.Labels, argov1alpha1.LabelKeyApplicationSetOrphaned)
			delete(app.Annotations, argov1alpha1.AnnotationKeyApplicationSetOrphanedBy)
			if patchErr := r.Patch(ctx, app, patch); patchErr != nil {
				logCtx.Errorf("Failed to remove orphaned label from application %s: %v", app.Name, patchErr)
				return patchErr
			}
			logCtx.Infof("Removed orphaned label from application %s (back in generator output)", app.Name)
		}
	}
	return nil
}
